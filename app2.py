...
def check_video_for_phrase_and_consistency(video_key, video_url, target_phrase):
    ...
    while cap.isOpened():
        ...
        if target_phrase in text:
            #print(f"Frase encontrada no vídeo {video_key}, frame {frame_count}")
            #log_message(f"Possível problema no vídeo {video_key}, {video_url}")
            #log_message(f"Realizando sintonia")

            #obter(video_key[:2], channel )    
            error_vids('Sem sinal no video',video, video_url, channel)

            break  # Parar de verificar após encontrar a frase
...
def error_vids(error, key, video, channel): # Deve consertar o erro de não achar o similares
...
def main():
    while True:  # Loop infinito para rodar indefinidamente
        ...
        # Pausar por 1 hora (3600 segundos) antes de analisar novamente
        print("Aguardando 1 hora antes da próxima análise...")
        time.sleep(3600/2)
